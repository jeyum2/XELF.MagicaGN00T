	...
	public sealed class MagicaVoxelParser {
		...
		public bool LoadModel(string absolutePath, VoxModel output, Logger logger) {
			...
			for (int i = 0; i < output.voxelFrames.Count; i++) {
				...
				if (!output.EnablesTransparent || !frame.Any(output.palette, c => c.a < 1)) {
			...
			for (int l = 1; l < output.maxLOD; l++) {
				for (int i = 0; i < output.voxelFrames.Count; i++) {
					logger?.Invoke($"frame={i}/{output.voxelFrames.Count}, lod={l}/{output.maxLOD}: {name}");
					...
					var scale = new Vector3(
						output.modelScale * output.voxelFrames[i].VoxelsWide / size.X,
						output.modelScale * output.voxelFrames[i].VoxelsTall / size.Y,
						output.modelScale * output.voxelFrames[i].VoxelsDeep / size.Z);
					var current = previous.ToSmaller();
					if (!output.EnablesTransparent || !current.Any(output.palette, c => c.a < 1)) {
						...
						mesher.MeshVoxelData(scale, current, output.palette,
							materials, output.origin, m.opaque);
						...
						mesher.MeshVoxelData(scale, current.Where(output.palette, c => c.a >= 1), output.palette,
							materials, output.origin, m.opaque);
						mesher.MeshVoxelData(scale, current.Where(output.palette, c => c.a < 1), output.palette,
							materials, output.origin, m.transparent);
						output.meshes[i].LODs.Add(m);
					}
					dataList[i] = current;
				}
